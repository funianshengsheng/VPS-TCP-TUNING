...
SCRIPT_VERSION="6.3.0-standalone"
...
baseline_sysctl_value() {
    ...
    [ -s "$SNAPSHOT_MODE" ] && snapshot_mode=$(cat "$SNAPSHOT_MODE" 2>/dev/null || echo "legacy")
    if [ "$snapshot_mode" = "fresh" ] && [ -s "$SYSCTL_STATE" ]; then
        setting=$(grep -F -m1 "${key}=" "$SYSCTL_STATE" 2>/dev/null || true)
        if [ -n "$setting" ]; then
            printf '%s\n' "${setting#*=}"
            return 0
        fi
...
select_tcp_buffer_defaults() {
    ...
    local rmem_value wmem_value
    local rmem_min rmem_default ignored_rmem_max
    local wmem_min wmem_default ignored_wmem_max
    local desired_rmem_default=131072
    local desired_wmem_default=65536

    rmem_value=$(baseline_sysctl_value net.ipv4.tcp_rmem 2>/dev/null || echo "4096 131072 6291456")
    wmem_value=$(baseline_sysctl_value net.ipv4.tcp_wmem 2>/dev/null || echo "4096 16384 4194304")
    read -r rmem_min rmem_default ignored_rmem_max <<< "$rmem_value"
    read -r wmem_min wmem_default ignored_wmem_max <<< "$wmem_value"

    if ! [[ "$rmem_min" =~ ^[0-9]+$ ]] || [ "$rmem_min" -le 0 ]; then rmem_min=4096; fi
    if ! [[ "$rmem_default" =~ ^[0-9]+$ ]] || [ "$rmem_default" -le 0 ]; then rmem_default=131072; fi
    if ! [[ "$wmem_min" =~ ^[0-9]+$ ]] || [ "$wmem_min" -le 0 ]; then wmem_min=4096; fi
    if ! [[ "$wmem_default" =~ ^[0-9]+$ ]] || [ "$wmem_default" -le 0 ]; then wmem_default=16384; fi
    [ "$rmem_min" -gt "$buffer_bytes" ] && rmem_min=4096
    [ "$wmem_min" -gt "$buffer_bytes" ] && wmem_min=4096

    # 高带宽代理连接提高初始 socket 缓冲，减少起速阶段的反复扩容；低内存和建站高并发保持克制。
    if [ "$profile" != "website" ] && [ "$bandwidth" -ge 500 ] && [ "$memory_mb" -ge 1024 ]; then
        desired_rmem_default=262144
        desired_wmem_default=262144
    elif [ "$memory_mb" -le 256 ]; then
        desired_rmem_default=$rmem_default
        desired_wmem_default=$wmem_default
    fi

    [ "$rmem_default" -lt "$desired_rmem_default" ] && rmem_default=$desired_rmem_default
    [ "$wmem_default" -lt "$desired_wmem_default" ] && wmem_default=$desired_wmem_default
    [ "$rmem_default" -lt "$rmem_min" ] && rmem_default=$rmem_min
    [ "$wmem_default" -lt "$wmem_min" ] && wmem_default=$wmem_min
...
calculate_tcp_notsent_lowat() {
    ...
    if [ "$bandwidth" -ge 500 ]; then
        if [ "$profile" = "website" ]; then
            lowat=1048576
        else
            lowat=4194304
        fi
    elif [ "$bandwidth" -ge 100 ]; then
        lowat=1048576
    fi

    # 未发送队列不超过发送窗口的 1/8，兼顾高吞吐供给和多连接场景的内存占用。
    maximum_lowat=$((buffer_bytes / 8))
    [ "$lowat" -gt "$maximum_lowat" ] && lowat=$maximum_lowat
    [ "$lowat" -lt 131072 ] && lowat=131072
    echo "$lowat"
}
...
calculate_profile_buffer_size() {
    ...
    case "$profile" in
        website)
            ...
            ;;
        *)
            required_mb=$(( (bdp_mb * 5 + 1) / 2 ))
            minimum_mb=16
            multiplier_label="2.5×BDP"
            memory_divisor=8
            hard_cap_mb=256
            ;;
    esac
    ...
    for dev in $(eligible_ifaces); do
        ...
        case "$root_kind" in
            mq)
                ...
                if [ -n "$leaf_kinds" ] && \
                   ! printf '%s\n' "$leaf_kinds" | tr ' ' '\n' | sed '/^$/d' | grep -Eqv '^fq$'; then
                    # 已经全部是 fq 时不重建 root，避免应用配置时冲掉活动队列与统计。
                    preserved=$((preserved + 1))
                elif [ -z "$leaf_kinds" ] || \
                     ! printf '%s\n' "$leaf_kinds" | tr ' ' '\n' | sed '/^$/d' | grep -Eqv '^(fq|fq_codel|pfifo_fast|pfifo|codel|sfq|noqueue)$'; then
            ...
            fq)
                preserved=$((preserved + 1))
                ;;
            ''|fq_codel|pfifo_fast|pfifo|codel|sfq|noqueue)
...
# Available memory: ${memory_mb} MB | TCP auto-tuning cap: ${buffer_mb} MB
...
EOF
    ...
    if ! printf 'profile=%s\nprofile_name=%s\nregion=%s\nregion_name=%s\nbandwidth_mbps=%s\nrtt_ms=%s\nmemory_mb=%s\nbuffer_mb=%s\ntcp_notsent_lowat=%s\ninitcwnd=%s\ninitrwnd=%s\nmss_clamp=%s\n' \
        ...
        "$memory_mb" "$buffer_mb" "$tcp_notsent_lowat" "$initcwnd" "$initrwnd" "$mss_clamp_enabled" > "$PROFILE_STATE"; then
...
EOF
    ...
    if [ "$actual_qdisc" = "fq" ] && [ "$actual_cc" = "bbr" ] && \
       ...
       [ "$qdisc_apply_failed" -eq 0 ]; then
        ...
        echo -e "${gl_zi}配置说明: ${profile_name}，${region_name}，${detected_bandwidth} Mbps / ${target_rtt_ms} ms，窗口 ${buffer_mb}MB，lowat ${tcp_notsent_lowat}，初始窗口 ${initcwnd_label}/${initrwnd_label}${gl_bai}"
    else
        apply_result=1
...
check_bbr_status() {
    ...
    if [ -s "$PROFILE_STATE" ]; then
        local profile_name saved_region saved_region_name bandwidth_mbps rtt_ms memory_mb buffer_mb saved_notsent_lowat
...
说明:
  ...
  - TCP 最大窗口按 BDP、用途和可用内存计算；代理约 2.5×BDP，建站约 2×BDP
  - tcp_notsent_lowat 按场景和带宽设置为 256 KiB～4 MiB，避免高吞吐时供给不足
