...
SCRIPT_VERSION="6.1.0-standalone"
...
select_target_rtt() {
    ...
    case "$profile" in
        optimize)
            echo "请输入优化机到主要落地机的实际 RTT。" >&2
            ;;
        landing)
            echo "请输入主要用户或前置优化机到落地机的实际 RTT。" >&2
            ;;
        website)
            echo "请输入主要访客到网站服务器的典型 RTT。" >&2
            ;;
    esac
    echo "提示：应使用目标链路延迟，不要使用最近 Speedtest 节点的延迟。" >&2
    while true; do
        read -e -p "目标 RTT（毫秒）[${default_rtt}]: " rtt
        rtt=${rtt:-$default_rtt}
        if [[ "$rtt" =~ ^[0-9]+$ ]] && [ "$rtt" -ge 1 ] && [ "$rtt" -le 2000 ]; then
            echo "$rtt"
            return 0
...
apply_tc_fq_now() {
        ...
        case "$root_kind" in
            mq)
                ...
                if [ -z "$leaf_kinds" ] || ! printf '%s\n' "$leaf_kinds" | tr ' ' '\n' | sed '/^$/d' | grep -Eqv '^(fq|fq_codel|pfifo_fast|pfifo|codel|sfq)$'; then
                    ...
                    tc qdisc replace dev "$dev" root mq 2>/dev/null && applied=$((applied + 1))
                else
    ...
    [ "$applied" -gt 0 ] && echo -e "${gl_lv}已对 $applied 个网卡应用 fq，且保留多队列 mq 结构${gl_bai}"
    [ "$preserved" -gt 0 ] && echo -e "${gl_huang}已保留 $preserved 个网卡的自定义根队列，避免破坏现有 QoS${gl_bai}"
    [ "$applied" -gt 0 ] || [ "$preserved" -gt 0 ] || echo -e "${gl_huang}未发现可应用 fq 的网卡${gl_bai}"
}
...
bbr_configure_direct() {
    ...
    echo ""
    ui_step 2 6 "测量瓶颈带宽、目标 RTT 与内存安全窗口"
    ...
    for d in /sys/class/net/*; do
        ...
        case "$root_kind" in
            mq)
                leaf_kinds=$(tc qdisc show dev "$dev" 2>/dev/null | awk '$2 != "mq" && $2 != "ingress" && $2 != "clsact" {print $2}' | sort -u | tr '\n' ' ')
                if [ -z "$leaf_kinds" ] || ! printf '%s\n' "$leaf_kinds" | tr ' ' '\n' | sed '/^$/d' | grep -Eqv '^(fq|fq_codel|pfifo_fast|pfifo|codel|sfq)$'; then
                    tc qdisc replace dev "$dev" root mq 2>/dev/null
                fi
                ;;
            ''|fq|fq_codel|pfifo_fast|pfifo|codel|sfq) tc qdisc replace dev "$dev" root fq 2>/dev/null ;;
        esac
    done
fi
...
cleanup_state_snapshot() {
    rm -f "$SYSCTL_STATE" "$QDISC_STATE" "$RPS_STATE" "$ROUTE_STATE" "$THP_STATE" \
        "$PROFILE_STATE" "$CONFLICT_STATE" "$SNAPSHOT_MODE" "$SNAPSHOT_READY" \
...
非交互示例:
  sudo env AUTO_MODE=1 TUNE_PROFILE=optimize BANDWIDTH_MBPS=1000 TARGET_RTT_MS=80 bash $0 apply
...
说明:
  ...
  - 非交互可设置 TUNE_PROFILE、BANDWIDTH_MBPS、TARGET_RTT_MS
