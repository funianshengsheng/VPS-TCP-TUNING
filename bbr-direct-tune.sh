...
SCRIPT_VERSION="6.4.1-standalone"
...
select_network_region() {
    ...
    if [ -n "$requested_region" ]; then
        ui_warn "SERVER_REGION/TUNE_REGION 无效，已改用地区选择" >&2
    fi
    ...
    echo "" >&2
    ui_section "选择主要链路地区" >&2
    if [ "$profile" = "optimize" ]; then
        echo "请选择主要远端或落地机所在地区：" >&2
    else
        echo "请选择服务器机房所在地区（按中国大陆主要访问方向估算）：" >&2
    fi
    echo "1. 亚洲      香港、日本、新加坡、韩国等（约 80ms）" >&2
    echo "2. 美洲      美国、加拿大等（约 180ms）" >&2
    echo "3. 欧洲      德国、荷兰、英国等（约 220ms）" >&2
    echo "4. 其他地区  大洋洲、中东、非洲或跨地区链路（约 200ms）" >&2
    echo "" >&2
...
select_target_rtt() {
    ...
    if [ -n "$rtt" ]; then
        ui_warn "TARGET_RTT_MS 无效，已改用地区基线" >&2
    fi
    ...
    ui_info "按$(region_label "$region")计算目标 RTT" >&2
    echo "$default_rtt"
}
...
detect_cpu_count() {
    local cpu_count quota period quota_count
...
calculate_initial_rwnd() {
    ...
    if [[ "${INIT_CWND:-}" =~ ^[0-9]+$ ]] && [ "$initcwnd" -gt 0 ]; then
        echo "$initcwnd"
        return 0
    fi
...
remember_sysctl_conf_edit_path() {
    ...
    if [ -s "$SYSCTL_CONFLICT_PATH_STATE" ]; then
        ...
        if [ "$recorded_path" != "$edit_path" ]; then
            ui_error "/etc/sysctl.conf 的实际目标已变化：$recorded_path -> $edit_path"
            ui_info "请先执行 restore 还原原目标，再处理新的 sysctl.conf"
            return 1
        fi
        return 0
    fi
...
check_and_clean_conflicts() {
    ...
    local timestamp suffix=0 disabled_count=0
    local key_regex

    key_regex=$(managed_sysctl_key_regex)
    if ! touch "$CONFLICT_STATE" 2>/dev/null; then
        ui_error "无法写入冲突恢复记录：$CONFLICT_STATE"
        return 1
    fi
    ...
    if [ "$has_sysctl_conflict" -eq 1 ]; then
        ...
        for key in "${TUNED_SYSCTL_KEYS[@]}"; do
            ...
            if ! sed -i -E "s|^([[:space:]]*-?[[:space:]]*${escaped_key}[[:space:]]*=)|# bbr-direct-tune disabled: \\1|" "$sysctl_conf_path"; then
                ui_error "无法清理 /etc/sysctl.conf 中的冲突项"
                return 1
            fi
        done
        ui_success "已注释 /etc/sysctl.conf 中的冲突项"
    fi
    ...
    for f in "${conflicts[@]}"; do
        ...
        if ! printf '%s|%s\n' "$f" "$disabled_file" >> "$CONFLICT_STATE"; then
            ui_error "无法记录已停用配置，正在回滚：$f"
            mv "$disabled_file" "$f" 2>/dev/null || ui_error "回滚失败，配置保留在：$disabled_file"
            return 1
        fi
        disabled_count=$((disabled_count + 1))
        ui_success "已停用：$f -> $disabled_file"
    done

    chmod 600 "$CONFLICT_STATE" 2>/dev/null || true
    [ "$disabled_count" -gt 0 ] && ui_info "restore 会按记录恢复上述 ${disabled_count} 个文件"
    return 0
}
...
apply_tc_fq_now() {
    ...
    for dev in $(eligible_ifaces); do
        ...
        case "$root_kind" in
            mq)
                ...
                # mq 叶队列可能带有驱动或管理员参数，无法无损快照时不重建。
                if [ -z "$leaf_kinds" ] || printf '%s\n' "$leaf_kinds" | tr ' ' '\n' | sed '/^$/d' | grep -Eqv '^fq$'; then
                    ui_warn "网卡 $dev 使用 mq / leaf ${leaf_kinds:-未知}，为避免破坏多队列参数已保留"
                fi
            ...
            ''|pfifo_fast)
                ...
                ;;
            *)
                ui_warn "网卡 $dev 当前为 ${root_kind:-未知}，无法无损保存参数，已保留并使用内核 TCP pacing"
                preserved=$((preserved + 1))
                ;;
        esac
    done
    ...
    [ "$applied" -gt 0 ] && ui_success "已对 $applied 个无参数简单根队列应用 fq"
    [ "$preserved" -gt 0 ] && ui_info "已保留 $preserved 个无需重建或不可无损恢复的网卡队列"
...
apply_default_route_initial_window() {
    ...
    [[ "$initcwnd" =~ ^[0-9]+$ ]] && [ "$initcwnd" -gt 0 ] && route_metrics+=(initcwnd "$initcwnd")
    [[ "$initrwnd" =~ ^[0-9]+$ ]] && [ "$initrwnd" -gt 0 ] && route_metrics+=(initrwnd "$initrwnd")
    ip route replace $clean_route "${route_metrics[@]}" >/dev/null 2>&1
}
...
bbr_configure_direct() {
    ...
    if ! ensure_sysctl_snapshot_key net.ipv4.tcp_retries2; then
        ui_error "未能补充保存 tcp_retries2 原值，已停止应用配置"
        return 1
    fi
    cleanup_legacy_runtime_before_apply
    ...
    ui_success "链路地区: $region_name"
    ui_info "检测到可用内存上限: ${memory_mb}MB"
    ui_info "检测到在线 CPU: ${cpu_count} 核"
    check_and_suggest_swap

    echo ""
    ui_step 2 6 "测量瓶颈带宽并按地区计算窗口"
    ...
    echo -e "${gl_lv}✅ socket 初始缓冲: rmem=${tcp_rmem_default} / wmem=${tcp_wmem_default} bytes${gl_bai}"
    echo -e "${gl_lv}✅ 未发送队列阈值: ${tcp_notsent_lowat} bytes${gl_bai}"
    echo -e "${gl_lv}✅ 路由初始窗口: initcwnd=${initcwnd_label} / initrwnd=${initrwnd_label}${gl_bai}"
    sleep 2
...
EOF
    ...
    if ! printf 'profile=%s\nprofile_name=%s\nregion=%s\nregion_name=%s\nbandwidth_mbps=%s\nrtt_ms=%s\nmemory_mb=%s\ncpu_count=%s\nbuffer_mb=%s\ntcp_rmem=%s %s %s\ntcp_wmem=%s %s %s\ntcp_notsent_lowat=%s\ntcp_retries2=%s\nsomaxconn=%s\ntcp_max_syn_backlog=%s\nnetdev_max_backlog=%s\ninitcwnd=%s\ninitrwnd=%s\nmss_clamp=%s\n' \
        ...
        "$netdev_backlog" "$initcwnd" "$initrwnd" "$mss_clamp_enabled" > "$PROFILE_STATE"; then
    ...
    if ! apply_default_route_initial_window "$initcwnd" "$initrwnd"; then
        ui_warn "默认 IPv4 路由未能恢复或应用 initcwnd/initrwnd；其余调优继续应用"
    fi
...
if command -v ip >/dev/null 2>&1; then
    ...
    if [ -n "$current_route" ]; then
        ...
        [[ "$INITCWND" =~ ^[0-9]+$ ]] && [ "$INITCWND" -gt 0 ] && route_metrics+=(initcwnd "$INITCWND")
        [[ "$INITRWND" =~ ^[0-9]+$ ]] && [ "$INITRWND" -gt 0 ] && route_metrics+=(initrwnd "$INITRWND")
        ip route replace $clean_route "${route_metrics[@]}" >/dev/null 2>&1 || true
    fi
fi
...
EOF
    ...
    local actual_initcwnd_label
    local actual_initrwnd_label
    [ -n "$actual_initcwnd" ] || actual_initcwnd=0
    [ -n "$actual_initrwnd" ] || actual_initrwnd=0
    actual_initcwnd_label=$actual_initcwnd
    actual_initrwnd_label=$actual_initrwnd
    [ "$actual_initcwnd" -eq 0 ] && actual_initcwnd_label="内核默认"
    [ "$actual_initrwnd" -eq 0 ] && actual_initrwnd_label="内核默认"
    ...
    if [ "$actual_wmem" = "$buffer_bytes" ] && [ "$actual_core_wmem" = "$buffer_bytes" ]; then
        echo -e "发送缓冲区: ${gl_lv}${buffer_mb}MB ✓${gl_bai}"
    else
        echo -e "发送缓冲区: ${gl_huang}${actual_wmem_mb}MB (期望: ${buffer_mb}MB) ⚠${gl_bai}"
    fi
    
    if [ "$actual_rmem" = "$buffer_bytes" ] && [ "$actual_core_rmem" = "$buffer_bytes" ]; then
        echo -e "接收缓冲区: ${gl_lv}${buffer_mb}MB ✓${gl_bai}"
    else
        echo -e "接收缓冲区: ${gl_huang}${actual_rmem_mb}MB (期望: ${buffer_mb}MB) ⚠${gl_bai}"
    fi

    if [ "$actual_tcp_rmem" = "$expected_tcp_rmem" ] && [ "$actual_tcp_wmem" = "$expected_tcp_wmem" ]; then
        echo -e "TCP 缓冲三元组: ${gl_lv}${expected_tcp_rmem} / ${expected_tcp_wmem} ✓${gl_bai}"
    else
        echo -e "TCP 缓冲三元组: ${gl_huang}${actual_tcp_rmem:-未知} / ${actual_tcp_wmem:-未知} ⚠${gl_bai}"
    fi

    if [ "$actual_notsent_lowat" = "$tcp_notsent_lowat" ]; then
        echo -e "未发送队列阈值: ${gl_lv}${tcp_notsent_lowat} bytes ✓${gl_bai}"
    else
        echo -e "未发送队列阈值: ${gl_huang}${actual_notsent_lowat:-未知} (期望: ${tcp_notsent_lowat}) ⚠${gl_bai}"
    fi

    if [ "$actual_slow_start_after_idle" = "0" ]; then
        echo -e "idle 后慢启动: ${gl_lv}关闭 ✓${gl_bai}"
    else
        echo -e "idle 后慢启动: ${gl_huang}${actual_slow_start_after_idle:-未知} (期望: 0) ⚠${gl_bai}"
    fi

    if [ "$actual_tcp_retries2" = "$tcp_retries2" ]; then
        echo -e "失败重试上限: ${gl_lv}${tcp_retries2} ✓${gl_bai}"
    else
        echo -e "失败重试上限: ${gl_huang}${actual_tcp_retries2:-未知} (期望: ${tcp_retries2}) ⚠${gl_bai}"
    fi

    if [ "$actual_somaxconn" = "$somaxconn" ] && [ "$actual_syn_backlog" = "$syn_backlog" ] && \
       [ "$actual_netdev_backlog" = "$netdev_backlog" ]; then
        echo -e "积压队列: ${gl_lv}${somaxconn}/${syn_backlog}/${netdev_backlog} ✓${gl_bai}"
    else
        echo -e "积压队列: ${gl_huang}${actual_somaxconn:-未知}/${actual_syn_backlog:-未知}/${actual_netdev_backlog:-未知} (期望: ${somaxconn}/${syn_backlog}/${netdev_backlog}) ⚠${gl_bai}"
    fi

    if [ "$actual_initcwnd" = "$initcwnd" ] && [ "$actual_initrwnd" = "$initrwnd" ]; then
        echo -e "初始窗口: ${gl_lv}cwnd ${initcwnd_label} / rwnd ${initrwnd_label} ✓${gl_bai}"
    else
        echo -e "初始窗口: ${gl_huang}cwnd ${actual_initcwnd_label} / rwnd ${actual_initrwnd_label} (期望: ${initcwnd_label}/${initrwnd_label}) ⚠${gl_bai}"
    fi
    ...
    if [ "$actual_qdisc" = "fq" ] && [ "$actual_cc" = "bbr" ] && \
       ...
       [ "$actual_netdev_backlog" = "$netdev_backlog" ] && \
       [ "$actual_initcwnd" = "$initcwnd" ] && [ "$actual_initrwnd" = "$initrwnd" ] && \
       [ "$qdisc_apply_failed" -eq 0 ]; then
        echo -e "${gl_lv}✅ BBR 自适应网络调优配置完成并已生效！${gl_bai}"
        echo -e "${gl_zi}配置说明: ${profile_name}，${region_name}，${detected_bandwidth} Mbps / ${target_rtt_ms} ms，窗口 ${buffer_mb}MB，lowat ${tcp_notsent_lowat}，队列 ${somaxconn}/${syn_backlog}/${netdev_backlog}${gl_bai}"
    else
        apply_result=1
        ...
        case "$qdisc_kind" in
            ...
            mq)
                ...
                if [ "$state_version" != "v2" ]; then
                    ui_warn "旧版 mq 快照缺少叶队列参数，无法验证精确恢复"
                    failed_count=$((failed_count + 1))
                fi
                ;;
            none|''|pfifo_fast)
...
show_network_counters() {
    ...
    if read -r out retrans < <(read_tcp_retrans_counters); then
        if [ "$out" -gt 0 ]; then
            ...
            printf '%-16s %s / %s（%s）\n' "TCP 重传段/发出段" "$retrans" "$out" "$retrans_ratio"
        else
            printf '%-16s %s\n' "TCP 重传段/发出段" "无有效样本"
        fi
    else
    ...
    if read -r processed softnet_dropped < <(read_softnet_counters); then
        printf '%-16s %s / %s\n' "softnet 处理/丢弃" "$processed" "$softnet_dropped"
    else
        printf '%-16s %s\n' "softnet 处理/丢弃" "无法检查"
    fi
    ...
    if command -v tc >/dev/null 2>&1; then
        for dev in $(eligible_ifaces); do
            if read -r qdisc_packets qdisc_dropped qdisc_requeues < <(read_qdisc_counters "$dev"); then
                printf '%-16s 发送 %s / 丢弃 %s / 重入队 %s\n' "qdisc ${dev}" "$qdisc_packets" "$qdisc_dropped" "$qdisc_requeues"
            else
                printf '%-16s %s\n' "qdisc ${dev}" "无法检查"
            fi
            qdisc_shown=$((qdisc_shown + 1))
        done
        [ "$qdisc_shown" -gt 0 ] || printf '%-16s %s\n' "qdisc" "未发现可管理网卡"
    else
        printf '%-16s %s\n' "qdisc" "无法检查（缺少 tc）"
    fi
...
check_bbr_status() {
    ...
    printf '%-16s %s\n' "发送缓冲区" "$tcp_wmem"
    printf '%-16s %s\n' "接收缓冲区" "$tcp_rmem"
    printf '%-16s %s / %s\n' "core 发送/接收上限" "$core_wmem_max" "$core_rmem_max"
    printf '%-16s %s\n' "未发送队列阈值" "$tcp_notsent_lowat"
    printf '%-16s %s\n' "idle 后慢启动" "$tcp_slow_start_after_idle"
    printf '%-16s %s\n' "tcp_retries2" "$tcp_retries2"
    printf '%-16s %s / %s / %s\n' "listen/SYN/netdev" "$somaxconn" "$syn_backlog" "$netdev_backlog"
    printf '%-16s %s\n' "活动 BBR 连接" "$active_bbr_sockets"
    ...
    if [ -s "$PROFILE_STATE" ]; then
        ...
        [ -n "$saved_region_name" ] && printf '%-16s %s\n' "链路地区" "$saved_region_name"
        printf '%-16s %s Mbps / %s ms\n' "目标链路" "${bandwidth_mbps:-未知}" "${rtt_ms:-未知}"
        printf '%-16s %s 核 / %s MB / %s MB\n' "CPU/内存/窗口" "${cpu_count:-未知}" "${memory_mb:-未知}" "${buffer_mb:-未知}"
        [ -n "$saved_notsent_lowat" ] && printf '%-16s %s bytes\n' "配置 lowat" "$saved_notsent_lowat"
    fi
...
show_help() {
    cat << EOF
BBR 多场景智能调优独立版 v${SCRIPT_VERSION}
...
非交互示例:
  sudo env AUTO_MODE=1 TUNE_PROFILE=optimize SERVER_REGION=america BANDWIDTH_MBPS=1000 bash $0 apply

说明:
  - 首次应用会在 ${STATE_DIR} 保存恢复快照
  - 会写入 ${SYSCTL_CONF}
  - 会写入 ${MODULES_CONF}，确保 tcp_bbr 开机加载
  - 自动注释 /etc/sysctl.conf 冲突行，并整份停用 /etc/sysctl.d 冲突文件
  - 停用路径会写入恢复映射，执行 restore 时精确还原
  - 会创建 ${PERSIST_SCRIPT}
  - 支持 systemd、OpenRC 和 SysV 重启持久化
  - 支持优化机、落地机、建站机三种场景
  - 根据亚洲、美洲、欧洲或其他地区自动采用 80/180/220/200 ms 链路基线
  - TCP 最大窗口按 BDP、用途和可用内存计算；代理约 3×BDP、最高 512 MiB，建站约 2×BDP
  - listen/SYN/netdev backlog 按带宽、CPU 与内存递增，满足条件时最高 65536
  - tcp_notsent_lowat：代理按带宽取 128 KiB～1 MiB、建站 16 KiB，兼顾单流吞吐与响应
  - tcp_retries2=8 会缩短持续断网时的失败等待，不会降低物理链路产生的重传
  - 保留 tcp_slow_start_after_idle=0，空闲长连接恢复时继续由 BBR + fq pacing 控制发送
  - 默认保留调优前的 initcwnd/initrwnd；没有原值时使用内核默认，避免放大首轮突发
  - 非交互可设置 TUNE_PROFILE、SERVER_REGION、BANDWIDTH_MBPS；TARGET_RTT_MS 可覆盖地区基线
  - 高级覆盖可设置 INIT_CWND、INIT_RWND（范围 10-32）；仅建议在实测无额外重传时使用
  ...
  - 应用前会预检 BBR、fq 与 tc；已有带参数、多队列或自定义 qdisc 不会被破坏性重建
  - status 显示累计 TCP 重传、softnet 丢弃及 qdisc 丢弃/重新入队，便于前后对比
  - 仅使用 Ookla 官方 Speedtest CLI，不覆盖同名的其他客户端
  - 临时下载的 Speedtest 必须通过固定 SHA-256 校验，并会在测速结束后自动清理
